...
Original file is located at
    https://colab.research.google.com/drive/1WIriCiYFGsNoI4klelQp8Ks3pAM8BFlm
"""
...
'''
1 - Faça uma consulta que retorne todos os cargos distintos
'''
funcionarios.select('cargo').distinct().show()

'''
2 - Faça uma consulta que retorne todos os nomes e escolaridades de
quem tem MBA
'''
funcionarios.select('nome','escolaridade')\
.where(funcionarios.escolaridade == "MBA").show()

'''
3 - Faça uma consulta de todos os nomes e salários de quem ganha acima de 30.000
'''
funcionarios.select('nome','salario')\
.where(funcionarios.salario > 30000).show()

'''
4 - Liste todos os departamentos os e suas respectivas notas médias
'''
funcionarios.groupby('departamento').agg(F.round(F.avg('nota'),1).alias('nota_media')).orderBy('nota_media', ascending=False).show()

'''
5 - Liste todas as cidades e suas respectivas idades médias
'''
funcionarios.groupby('cidade').agg(F.round(F.avg('idade'),1)\
                                   .alias('nota_media')).show()

"""# Operações que modificam o df spark"""

'''
Adicionar uma coluna chamada bonus que é 10% do salário.
'''
funcionarios.withColumn('bonus', F.round(F.col('salario') * 0.1,2)).show()

'''
Atualizar a coluna salario para incluir o bônus (incrementar o salário em 10%)
'''
funcionarios.withColumn('salario', F.round(F.col('salario') * 1.1, 2)).show()

'''
Criando uma coluna com base em outras duas
Imagine que você deseja criar uma coluna que multiplice o valor do salário pela
nota
'''
funcionarios.withColumn('salario * nota',\
                        F.round(F.col('salario') * F.col('nota'),2)).show()

'''
Criar uma coluna categoria_idade que classifica os funcionários como "Jovem"
se a idade for menor que 30, e "Sênior" caso contrário.
'''
funcionarios.withColumn('categoria_idade',\
                F.when(F.col('idade') < 30, 'Jovem').otherwise('Senior')).show()

'''
Categorizar o salário em "Baixo", "Médio" ou "Burguês".
< 15000 - Baixo, entre 15000 e 30000 (inclusive) - Médio e maior que
30.000 (Burgês)
'''
funcionarios.withColumn('Classe', F.when(F.col('salario') < 15000, 'Baixo')\
                        .when((F.col('salario') >= 15000) & (F.col('salario')\
                                <= 30000), 'Médio').otherwise('Burguês')).show()

'''
Converter a coluna idade de IntegerType para StringType.
'''
funcionarios.withColumn('idade', F.col('idade').cast('string')).printSchema()

'''
Criar uma coluna data_atual com a data do sistema.
'''
funcionarios.withColumn('data_atual',F.current_date()).show()

'''
Criar uma coluna cargo_departamento concatenando o cargo e o departamento.
'''
funcionarios.withColumn('cargo_departamento', F.concat(F.col('cargo'),\
                                  F.lit(' -- '), F.col('departamento'))).show()

'''
Faça uma consulta que retorne um df pyspak de uma linha com uma coluna
contendo o valor da idade média dentro de si e guarde o resultado em um objeto a
'''
a = funcionarios.agg(F.avg('idade').alias('idade_media'))

'''
Exiba o que está armazenado no objeto a
'''
a.show()

'''
Extraia o valor contifo no ojeto a
e armazene em b
'''
b = a.collect()[0]['idade_media']

'''
Verifique o tipo armazenado em b
'''
type(b)

# Extraia o valor de dentro do ojeto obtido com o bloco anterior
(funcionarios.agg(F.avg('idade').alias('idade_media')).collect()[0]['idade_media'])

'''
Selecione nome e idade de todos os funcionários idade acima da média
'''
funcionarios.select('nome', 'idade').where(funcionarios.idade > \
 (funcionarios.agg(F.avg('idade').alias('idade_media'))\
.collect()[0]['idade_media'])).show()

'''
Liste todos os nomes e notas dos funcionarios com nota
acima da média
'''
funcionarios.select('nome', 'nota').where(funcionarios.nota > \
 (funcionarios.agg(F.avg('nota').alias('nota_media'))\
.collect()[0]['nota_media'])).show()

"""# Banco Locadora (eu não aguento mais esse banco)"""
...
2 - Liste os estados diferentes presentes na tabela cliente.
'''
cliente.select('estado').distinct().show()
...
# Alternativa
cliente.select('nome','sexo','estadocivil')\
.where((cliente.sexo == 'F') & (cliente.estadocivil == 'S')).show()
...
4 - Liste todos os clientes cujo nome começa com a letra 'A'.
'''
cliente.select('*').where(cliente.nome.startswith('A')).show()
...
5 - Liste todos os carros que são da marca 'Ford' ou 'Fiat'.
'''
carro.join(marca, carro.codmarca == marca.codmarca)\
.where((marca.marca == 'Fiat') | (marca.marca == 'Ford'))\
.select('modelo','marca').show()
...
.join(carro, aluguel.codcarro == carro.codcarro)\
.where(carro.modelo == 'Onix').select('nome','modelo').show()
...
.groupBy('modelo').agg(F.count('codaluguel').alias('qtde_alugueis'))\
...
'''
aluguel.join(cliente, aluguel.codcliente == cliente.codcliente)\
.groupBy(cliente.nome.alias('nome_cliente'))\
...
# Alternativa
cliente.select('nome',F.when(cliente.estadocivil == 'S'\
                             , 'Maybe_Happy or Maybe_Sad')\
.when(cliente.estadocivil == 'C' ,'Casado').alias('status_civil')).show()
...
aluguel.join(cliente, aluguel.codcliente == cliente.codcliente)\
.join(carro, aluguel.codcarro == carro.codcarro)\
.where((F.month(aluguel.data_aluguel) == 4) & \
(F.year(aluguel.data_aluguel) == 2023))\
.select(cliente.nome.alias("nome_cliente"), \
carro.modelo.alias("modelo_carro"), aluguel.data_aluguel).show()
